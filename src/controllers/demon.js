...
export const DemonCreate = async (req, res, next) => {
  try {
    const { name, holder, place, verifier, creator, video, level_id, minimal_percent, length, objects, version, description } = req.body
    ...
    return res.status(200).send({ data: { list: demonsNewList, demon: demon }, message: 'New demon created' })
  } catch(error) {
    next(error)
  }
}
...
export const DemonListUpdate = async (req, res, next) => {
  try {
    ...
    const userAdmin = await User.findById(req.user)
    if (userAdmin.role < 2) res.status(403).send({ error: 'Insufficient permissions to perform this action' })
    ...
    return res.status(200).send({ data: demonsNewList, message: 'List updated' })
  } catch (error) {
    next(error)
  }
}
...
export const GetDemonsList = async (req, res, next) => {
  try {
    ...
    return res.status(200).send({ data: demons, message: 'Demon list retrieved successfully' })
  } catch (error) {
    next(error)
  }  
}
