...
export const RecordSubmit = async (req, res, next) => {
	try {
		...
		if (percent < demon.minimal_percent)
			return res.status(400).send('Percent is lower than demon list percent')
		...
		const record = await Record.create({
			...
			demon_holder: demon.holder,
			description,
			video,
			user_submitter_id: user._id,
			user_submitter_name: user.username
		})
		if (!record)
			return res.status(400).send({ error: 'Failed to create record' })
		const profileRecord = await Record.findById(record._id).select(
			'-raw_footage'
		)
		...
		return res
			.status(200)
			.send({ data: record, message: 'New record submitted successfully' })
	} catch (error) {
		next(error)
	}
}
...
export const RecordUpdateStatus = async (req, res, next) => {
	try {
		...
		if (
			!demon.records.some(
				r =>
					r.demon_id === profileRecord.demon_id &&
					r.user_submitter_id === profileRecord.user_submitter_id
			)
		) {
		...
		return res
			.status(200)
			.send({ data: record, message: `Record status set to ${status}` })
	} catch (error) {
		next(error)
	}
}
...
export const RecordList = async (req, res, next) => {
	try {
		...
		return res
			.status(200)
			.send({ data: records, message: 'Record list retrieved successfully' })
	} catch (error) {
		next(error)
	}
}
