	...
	try {
		...
		if (!user) {
			...
			const list = await Demon.find({
				_id: user.records.map(r => r.demon_id)
			}).sort({ place: 1 })
			...
			if (user.banned) {
				if (user.ban_expire) {
					...
					if (dayjs().isBefore(banExpires)) {
						return res
							.status(403)
							.json({ error: 'Your account is temporarily banned' })
					} else {
						user.banned = false
					...
					return res
						.status(403)
						.json({ error: 'Your account is permanently banned' })
				}
			}
...
export const UserForceBan = async (req, res, next) => {
	try {
		...
		if (!adminUser || adminUser.role < 1) {
			return res
				.status(403)
				.json({ error: 'Insufficient permissions to perform this action' })
		}
		if (id === adminUser._id) {
